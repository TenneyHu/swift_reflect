...
class AlgorithmReflectSummary(Algorithm):
    def __init__(self,
        ...
        self.summary_params = agents["summary_params"]
    ...
    async def solve(self, idx: int, initial_state: StateHotpotQA, trial: int, max_trial: int, namespace: str, value_cache: dict = None):
        ...
        if trial < max_trial:
            ...
            summary_text = await self.summary_agent.act(
                model=self.model,
                reflections=trial_state.reflections + [reflection_text],
                namespace=namespace,
                request_id=f"idx{idx}-trial{trial}-summarize-{hash(trial_state)}",
                params=self.summary_params
            )
            trial_state = trial_state.clone(new_reflection=reflection_text, set_summary=summary_text)

            logger.info(f"Task {idx}, Trial {trial  }: Summary: {trial_state.summary}, length of reflections: {len(trial_state.reflections)}")
        else:
